...
namespace DroidFinestra.Helpers
{
    ...
    /// installing fonts (works on RT — private-loaded, not system). Every call returns a fresh
    ...
    public static class FontHelper
    {
        ...
        static FontHelper()
        {
            try
            {
                AddFont("app.fonts.Roboto-Regular.ttf");
                AddFont("app.fonts.Roboto-Medium.ttf");
                AddFont("app.fonts.Roboto-Bold.ttf");
        ...
        private static void AddFont(string resourceName)
        {
            try
            {
                ...
                using (var s = asm.GetManifestResourceStream(resourceName))
                {
                    ...
                    // ALSO register with GDI so TextRenderer/GDI can see the font WITHOUT installing it —
                    // the app's Labels/controls render with UseCompatibleTextRendering=false (GDI TextRenderer),
                    // which otherwise resolves `new Font("Roboto",…)` to a SYSTEM substitute. Shares the same
