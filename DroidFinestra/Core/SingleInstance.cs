...
namespace DroidFinestra.Core
{
    ...
    public static class SingleInstance
    {
        ...
        public static bool TryBecomeOwner()
        {
            ...
            _pipeName = AppIdentity.Id + ".Activate." + hash + "." + UserSid();

            bool createdNew;
            try { _mutex = new Mutex(true, @"Local\" + AppIdentity.Id + ".SingleInstance." + hash, out createdNew); }
        ...
        private static void Dispatch(Form manager)
        {
            try
            {
                Form target = (manager != null && !manager.IsDisposed) ? manager : null;
