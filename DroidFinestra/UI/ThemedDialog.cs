...
namespace DroidFinestra.UI
{
    ...
    public class ThemedDialog : Form
    {
        ...
        {
            _title = title ?? "";
            Text = _title;   // DroidFinestra: the painted header is not what the taskbar / Alt-Tab show
        ...
        /// <summary>DroidFinestra: stretch every row to the body width, then re-stack them top-to-bottom — a row
        /// whose height depends on its width (a wrapping note) gets its real height before the next row is placed.</summary>
        protected void StretchAndRestack()
        {
            int w = Body.Host.ClientSize.Width;
            int y = 6;
            foreach (Control r in Body.Host.Controls)
            {
                if (w > 10) r.Width = w;
                r.Top = y;
                y += r.Height;
