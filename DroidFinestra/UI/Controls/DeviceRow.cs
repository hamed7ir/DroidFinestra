...
{
    /// <summary>
    /// One device profile in the list — Finestra's ConnectionRow: a themed rounded card showing the name + a
    /// one-line summary, with Launch / Edit / Delete / Copy actions (Delete is Danger-red, never accent). Colors
    /// follow <see cref="ThemeHelper"/> and recolor live. Owner-painted so it looks identical on RT 8.1 and 10/11.
    /// </summary>
    public sealed class DeviceRow : Control
    {
        public DeviceProfile Profile { get; }
        public event Action<DeviceProfile> LaunchClicked;
        public event Action<DeviceProfile> EditClicked;
        public event Action<DeviceProfile> DeleteClicked;
        /// <summary>Copy button / right-click → Duplicate.</summary>
        public event Action<DeviceProfile> DuplicateClicked;
        /// <summary>Right-click → Copy command line.</summary>
        public event Action<DeviceProfile> CopyCommandClicked;

        private readonly RoundedButton _connect, _edit, _delete, _copy;

        public DeviceRow(DeviceProfile cp)
        {
            Profile = cp;
            Height = 78;
            ...
            _connect = Btn("Launch", RoundedButtonKind.Primary, () => LaunchClicked?.Invoke(Profile));
        ...
        private void ShowContextMenu(Point screen)
        {
            ...
            menu.Items.Add(new ToolStripMenuItem("Copy command line", null, (s, e) => CopyCommandClicked?.Invoke(Profile)));
            menu.Show(screen);
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            var glyph = TypeGlyph.Get(TypeGlyph.KindOf(Profile.Transport), tile);
            ...
            using (var nf = FontHelper.Ui(12f, FontStyle.Bold))
                TextRenderer.DrawText(g, Profile.DisplayName, nf, nameR, fg,
                    TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPrefix | TextFormatFlags.EndEllipsis);
            using (var sf = FontHelper.Ui(9.5f))
                TextRenderer.DrawText(g, Profile.Summary, sf, subR, sub,
                    TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPrefix | TextFormatFlags.EndEllipsis);
        }
    }
}
